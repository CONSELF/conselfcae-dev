...
namespace Foam
{
    namespace calcTypes
    {
        ...
        addToRunTimeSelectionTable(calcType, max, dictionary);
    }
}
...
void Foam::calcTypes::max::init()
{
    argList::validArgs.append("max");
    argList::validArgs.append("fieldName");
}


void Foam::calcTypes::max::preCalc
(
    const argList& args,
    const Time& runTime,
    const fvMesh& mesh
    ...
    if (fieldHeader.headerOk())
    {
        ...
        if (!processed)
        {
            FatalError
                ...
                << "No call to max for fields of type "
